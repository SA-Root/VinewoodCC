...
namespace VinewoodCC.NGen
{
    class NativeCodeGenenrator
    {
        ...
        {
            var last = CodeSegment[^1];
            if (last[^1] == ' ')
            {
                CodeSegment[^1] += string.Format("{0}:dword", qt.LValue.ID);
            }
            else
            {
                CodeSegment[^1] += string.Format(",{0}:dword", qt.LValue.ID);
        ...
        {
            OutputFile = string.Concat(path.AsSpan(0, path.LastIndexOf(".vcil")), ".asm");
            try
            {
